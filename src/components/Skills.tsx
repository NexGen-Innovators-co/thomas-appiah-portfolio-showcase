  ...
  const skillCategories = [
    {
      ...
      skills: ["Python", "JavaScript/TypeScript", "Dart", "C/C++", "Java", "SQL"],
    ...
    <section className="py-20 px-4 relative overflow-hidden bg-slate-900">
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="absolute inset-0" style={{
          backgroundImage: `linear-gradient(90deg, rgba(16, 185, 129, 0.3) 1px, transparent 1px),
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
          {skillCategories.map((category, index) => (
            <Card 
              key={index} 
              className="bg-slate-800 border-slate-700 hover:bg-slate-750 transition-all duration-300 hover:scale-105 shadow-lg"
            >
                ...
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill, skillIndex) => (
                    <Badge 
                      key={skillIndex} 
                      variant="secondary" 
                      className="bg-slate-700 text-white hover:bg-slate-600 transition-colors duration-200"
                    >
                      {skill}
                    </Badge>
                  ))}
